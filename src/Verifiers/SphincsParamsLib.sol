...
library SphincsParamsLib {
    ...
    /// @notice The UPSTREAM reference parameter set (h=22 d=2 k=7 a=19 w=8 l=43 target_sum=208,
    ///         3,688-byte blob).
    /// @dev    WARNING: on this branch `SphincsVerifier` has been retargeted and NO LONGER
    ///         implements this set — see `retargeted()`. This function is kept because it is the
    ///         set the upstream signer and the committed differential tests are built around;
    ///         do not use it to drive the local fixed verifier.
    function canonical() internal pure returns (Params memory) {
        return Params({h: 22, d: 2, k: 7, a: 19, logW: 3, l: 43, targetSum: 208});
    }

    /// @notice The parameter set the local fixed `SphincsVerifier` implements
    ///         (h=20 d=4 k=29 a=7 w=4 l=64 target_sum=96, 8,048-byte blob == SPHINCS_SIG_LEN).
    /// @dev    WOTS+C: l = len1 = ceil(8n/log2 w) = 64 only — there are no checksum chains;
    ///         targetSum (the digit-sum mean l*(w-1)/2) plays that role.
    function retargeted() internal pure returns (Params memory) {
        return Params({h: 20, d: 4, k: 29, a: 7, logW: 2, l: 64, targetSum: 96});
    }
