...
/// @notice Deploys `SphincsAccountFactory`, whose constructor in turn deploys the `SphincsAccount`
///         implementation. Binds to the already-deployed `SphincsStandardVerifier` (8,400-byte
///         standard-FORS / standard-WOTS+ blob).
...
///
///         The verifier address is DERIVED from its deploy salt rather than hardcoded, and the
///         script asserts it already has code: binding the factory to an address with no verifier
///         behind it would produce accounts that can never validate anything.
///
///         Idempotent: an already-deployed factory is skipped.
contract DeploySphincsAccount is Script {
    ...
    function run() external {
        ...
        // The verifier must already be on chain (deployed by DeployVerifiers.s.sol).
        address verifier = _predictDeterministicAddress(verifierSalt, type(SphincsStandardVerifier).creationCode);
        require(verifier.code.length != 0, "Deploy: standard verifier not deployed");
        ...
        console.log("SphincsStandardVerifier:         ", verifier);
